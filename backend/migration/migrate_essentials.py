#!/usr/bin/env python3
"""مهاجرت دادهٔ «فقط ضروری» از سوپابیس به بک‌اند اختصاصی (فاز ۱۰، کار #94).
...
می‌شوند (تصمیم مالک: «فقط ضروری» + «شروع تمیز فایل‌ها»).

به‌علاوه، جدول مرجع ساکن `card_bins` (نگاشت BIN → نام بانک؛ دادهٔ کاربر نیست) به‌صورت
پیش‌فرض منتقل می‌شود تا شماره‌کارت‌ها در اپ به نام بانک نگاشت شوند؛ با `--skip-reference`
می‌توان صرف نظر کرد.

اصول ثابت این مهاجرت:
  • ستون‌های حمل‌شونده = «ستون‌های تارگت ∩ ستون‌های سورس» منهای EXCLUDE.
    این یعنی هر ستونی که در اسکیمای جدید نیست (یا در سورس نیست) خودبه‌خود کنار می‌رود
    و ستون‌های تازهٔ تارگت مقدار پیش‌فرض خود را می‌گیرند — بدون شکستن با drift اسکیما.
  • national_id هرگز منتقل نمی‌شود (PII، حذف‌شده از کل برنامه).
  • members.avatar_url اجباراً NULL می‌شود (فایل‌ها از صفر؛ لینک‌های قدیمی استوریج بی‌اعتبارند).
  • password_hash و phone باید در سورس باشند وگرنه مهاجرت متوقف می‌شود (ورود بدون آن‌ها می‌شکند؛
    بک‌اند جدید هش‌های قدیمی را هنگام ورود موفق به Argon2id ارتقا می‌دهد).
  • ترتیب بار رعایت کلیدهای خارجی را تضمین می‌کند و همه در «یک تراکنش» انجام می‌شود.

نکتهٔ حیاتی RLS: جدول‌های accounts/subcategories/custom_categories/category_budgets در
مهاجرت 0002 با FORCE RLS بسته می‌شوند و آن‌گاه حتی مالک جدول هم بدون زمینهٔ خانواده
...
try:
    ...
    sys.stderr.write(
        "psycopg (v3) نصب نیست. این اسکریپت را از داخل venv بک‌اند اجرا کنید:\n"
        "    cd backend && pip install -e \".[dev]\"\n"
    )
    raise SystemExit(2)


# ── پیکربندی ثابت مهاجرت ────────────────────────────────────────────────────
...
#: جدول‌های مرجع ساکن سراسری (نه دادهٔ کاربر، نه خانواده‌محور، بیرون از RLS). با
#: --skip-reference می‌توان صرف نظر کرد. 0001 جدول را می‌سازد ولی ردیف نمی‌ریزد؛ بدون
#: این‌ها شماره‌کارت‌ها در اپ به نام بانک نگاشت نمی‌شوند.
REFERENCE_TABLES: tuple[str, ...] = (
    "card_bins",  # BIN شش‌رقمی → نام بانک (کلید اصلی: bin، نه id)
)
...
def get_urls() -> tuple[str, str]:
    ...
    if missing:
        raise SystemExit(
            "متغیرهای محیطی لازم ست نشده‌اند: " + "، ".join(missing) + "\n"
            "نمونه:\n"
...
def table_columns(conn: "psycopg.Connection", table: str) -> list[str]:
    """نام ستون‌های جدول به ترتیب ordinal (از information_schema)."""
...
def carry_columns(target_cols: list[str], source_cols: set[str]) -> list[str]:
    """ستون‌های حمل‌شونده: در تارگت هست، در سورس هم هست، و در EXCLUDE نیست (ترتیب تارگت حفظ می‌شود)."""
...
def plan_table(src: "psycopg.Connection", tgt: "psycopg.Connection", table: str) -> dict:
    """نقشهٔ مهاجرت یک جدول را می‌سازد و اعتبارسنجی می‌کند (بدون نوشتن)."""
    ...
    if not table_exists(src, table):
        # جدول ضروری در سورس نیست → با صفر ردیف رد می‌شویم (سناریوی بعید).
    ...
    if absent:
        raise SystemExit(
            f"ستون‌های حیاتی در سورس «{table}» نیستند: {absent}. "
            "بدون این‌ها مهاجرت متوقف شد (ورود می‌شکند)."
        )

    carry = carry_columns(tgt_cols, src_cols)
    pk = CONFLICT_KEY.get(table, "id")
    if pk not in carry:
        raise SystemExit(f"کلید «{pk}» برای «{table}» قابل حمل نیست؛ مهاجرت امن نیست.")
...
def copy_table(src: "psycopg.Connection", tgt: "psycopg.Connection", plan: dict) -> int:
    """ردیف‌های یک جدول را با ستون‌های نقشه منتقل می‌کند. تعداد درج‌شده را برمی‌گرداند."""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="مهاجرت دادهٔ «فقط ضروری» (فاز ۱۰).")
    ...
    ap.add_argument("--skip-reference", action="store_true",
                    help="جدول‌های مرجع ساکن (card_bins) را منتقل نکن.")
    args = ap.parse_args()
    ...
    print("در حال اتصال به سورس و تارگت…")
    with psycopg.connect(source_url) as src, psycopg.connect(target_url) as tgt:
        ...
        # اجرا شده و بار داده به‌عنوان مالک بسته می‌شود. (در dry-run فقط هشدار، چون نوشتنی نیست.)
        ...
        for p in plans:
            ...
            if p["missing_source"]:
                ...
                continue
            print(f"  {p['table']:<18} ردیف سورس={p['source_rows']:>6}  ستون‌ها={len(p['carry'])}{tag}")
        ...
        print("انجام شد ✓" if ok else "هشدار: شمارش برخی جدول‌ها کمتر از سورس است — بررسی کنید.")
    return 0
