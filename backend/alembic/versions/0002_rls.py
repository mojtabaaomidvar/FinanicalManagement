...
مرز چند-مستأجری در سطح دیتابیس: حتی اگر لایهٔ اپ فیلتر family_id را فراموش کند،
خود Postgres ردیف‌های خانواده‌های دیگر را نه می‌خواند و نه می‌نویسد.

دامنه (تصمیم مالک ۲۰۲۶-۰۹-۰۸): فقط جدول‌های «همیشه‌احرازشده» که پیش از داشتن
زمینهٔ خانواده هرگز خوانده نمی‌شوند. جدول‌های بوت‌استرپ هویت (members, families,
...
مکانیزم: هر درخواست احرازشده در همان تراکنش
...
FORCE لازم است چون اپ (فعلاً) با نقش مالک جدول‌ها متصل می‌شود و مالک به‌صورت
پیش‌فرض از RLS معاف است؛ FORCE مالک را هم مشمول می‌کند. (نقش کم‌دسترسی جدا =
سخت‌سازی اختیاری فاز ۱۱ وقتی هاست مشخص شد.)

⚠️ ترتیب بار دادهٔ فاز ۱۰: چون FORCE، بار data-only به‌عنوان همان نقش مشمول RLS
می‌شود و INSERTها رد می‌شوند. پس ترتیب درست:
    alembic upgrade 0001   →   بار pg_dump --data-only   →   alembic upgrade head
...
# عمداً بدون members/families/family_invites (بوت‌استرپ هویت) و بدون جدول‌های سراسری.
...
# محمول مشترک: fail-closed؛ NULLIF از خطای ''::uuid جلوگیری می‌کند و نبود GUC → NULL → رد.
...
def upgrade() -> None:
    for table in _TENANT_TABLES:
        ...
        # FORCE تا مالک جدول هم مشمول شود (اپ فعلاً با نقش مالک متصل است).
