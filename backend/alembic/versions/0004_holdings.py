...
کاربر مقدار را نگه می‌دارد و ارزش هر بار از قیمت روز بازار حساب می‌شود؛
هیچ مبلغی اینجا ذخیره نمی‌شود. این عمدی است: «۲ سکه» با گذشت زمان معنایش
...
۱) quantity از نوع Numeric(18,6) است نه Numeric(18,0). جدول accounts اعشار
   ندارد چون تومان صحیح است، ولی «۲٫۵ گرم طلا» یا «۰٫۰۳ بیت‌کوین» بدون اعشار
   اصلاً بیان نمی‌شود. شش رقم اعشار برای رمزارزهای گران هم کافی است.

۲) name کنار symbol ذخیره می‌شود — یعنی عکس لحظه‌ای نام. اگر بالادست نمادی
   را حذف یا تغییر نام دهد، ردیف کاربر همچنان خوانا می‌ماند («۲ سکه امامی»)
   حتی وقتی دیگر نمی‌شود قیمتش را گرفت. بدون این، فهرست دارایی‌ها به ردیف‌های
   بی‌نام قابل‌شناسایی‌نشدنی تبدیل می‌شد.

۳) kind ذخیره می‌شود تا موقع قیمت‌گذاری معلوم باشد سراغ کدام منبع برویم:
   gold/currency/crypto از اسنپ‌شات Gold_Currency می‌آیند ولی stock از فهرست
   AllSymbols. بدون kind باید در هر چهار فهرست دنبال نماد می‌گشتیم و یک
   هم‌نامی اتفاقی قیمت غلط می‌داد.
...
def upgrade() -> None:
    op.create_table(
        ...
        # مقدار صفر یا منفی دارایی نیست؛ کاربر باید ردیف را حذف کند نه صفر کند.
