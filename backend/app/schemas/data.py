"""قرارداد خروجی اندپوینت‌های خواندن داده (Pydantic v2) — فاز ۶.

اصول (هم‌راستا با schemas/auth.py):
- سازندهٔ صریح `.of(model)` هر فیلد را دستی می‌سازد؛ هرگز ستون ناخواسته‌ای درز نمی‌کند.
- نام فیلدها دقیقاً snake_case ستون‌های دیتابیس‌اند تا با ردیف‌های `mappers.ts`
...
- مبالغ (Numeric) مثل `FamilyOut.budget` به‌صورت «عدد» (float) سریال می‌شوند تا با
  انتظار فعلی کلاینت (`+r.amount`) سازگار بمانند. محدودهٔ دقیق float64 (تا ۲^۵۳)
  برای مبالغ ریالی خانواده کافی است؛ اگر روزی دقت بانکی لازم شد، کل قرارداد پول
  یک‌جا به رشته/واحد خرد تغییر می‌کند (تصمیم جداگانه).
...
class TransactionPhotoOut(BaseModel):
    """معادل آرایهٔ photos در list_transactions قدیمی: فقط id/url/caption."""
...
# ── پل پیامک (get_bridge: فقط token + member_id، یا null) ──
...
class PhotoOut(BaseModel):
    """ردیف کامل عکس — خروجی نوشتن add_tx_photo (خواندن نمای کوتاه
...
# ═══════════════════════════════════════════════════════════════
# بدنه‌های ورودی نوشتن (فاز ۷)
# ───────────────────────────────────────────────────────────────
# عمداً «سهل‌گیر» (فیلدها Optional با پیش‌فرض None): همهٔ اعتبارسنجی
# تجاری در لایهٔ سرویس با کدهای خطای پایدار (INVALID_TYPE، INVALID_AMOUNT،
# BANK_MISMATCH، …) انجام می‌شود تا نگاشت خطای کلاینت (RPC_ERROR_MAP) بی‌تغییر
# بماند. اگر اینجا اعتبارسنجی سخت بگذاریم، خطاها به‌جای کد دامنه، 422 Pydantic
# می‌شوند و قرارداد می‌شکند. تاریخ‌ها 'YYYY-MM-DD' و مبالغ عدد/رشته → Decimal.
# ═══════════════════════════════════════════════════════════════


class TransactionWrite(BaseModel):
    """ورودی افزودن/ویرایش تراکنش (هزینه/درآمد/انتقال)."""
...
class OccurrenceMark(BaseModel):
    """ثبت رسیدگی به یک سررسید تراکنش تکرارشونده."""
...
# ── ingest پیامک (فاز ۸): پارس «سمت سرور» ───────────────────
# برخلاف SmsItem/SmsBatch (که فیلدهای «پارس‌شده» را از کلاینت می‌گیرند)، این‌ها فقط
# متن خام را می‌گیرند و سرور خودش پارس می‌کند تا وب و اندروید یک نتیجهٔ یکسان بدهند.
class SmsIngestItem(BaseModel):
    """یک پیامک خام برای پارس سمت‌سرور (خواندن نیتیو اپ یا مسیر دستی آینده)."""

    raw_text: str | None = None
    sender: str | None = None  # فرستنده (اختیاری) — فقط کمکی برای تشخیص بانک
...
class BridgeIngest(BaseModel):
    """بدنهٔ webhook پل (جایگزین api/sms-webhook.js): {token, text, sender?}.

    احراز با «توکن پل» است، نه نشست کاربر؛ پس token در بدنه می‌آید (استثنای آگاهانه
    نسبت به قاعدهٔ «هویت فقط از Bearer»؛ این یک webhook ماشین-به-ماشین است)."""
...
class IngestResult(BaseModel):
    """خروجی bridge-ingest: تعداد پیامک واردشده (۰ یا ۱)."""

    ok: bool
    count: int


# ── آپلود فایل (فاز ۸): dataURL → استوریج خصوصی + URL امضاشده ──
# احراز با نشست Bearer است (نه توکن-در-بدنه مثل توابع سرورلس قبلی)؛ پس بدنه فقط
# خود تصویر را دارد. خروجی همان شکل قبلی {ok, url} است تا نگاشت کلاینت کم‌تغییر بماند.
class UploadIn(BaseModel):
    """بدنهٔ آپلود: تصویر به‌صورت dataURL base64 (png/jpeg/webp)."""

    image: str | None = None


class UploadOut(BaseModel):
    """خروجی آپلود: URL امضاشدهٔ قابل ذخیره در DB و رندر در <img>."""
...
class FamilySettingsUpdate(BaseModel):
    """فقط budget اعمال می‌شود؛ currency/dark برای سازگاری امضا پذیرفته و
...
class HoldingCreate(BaseModel):
    ...
    symbol/name/unit از همان ردیف قیمتی می‌آید که کاربر رویش زده، پس کلاینت
...
class HoldingUpdate(BaseModel):
    ...
    اگر کاربر بخواهد دارایی دیگری ثبت کند ردیف تازه می‌سازد — تغییر نماد یعنی
    ردیف دیگر معنای قبلی‌اش را ندارد و تاریخچه‌اش گمراه‌کننده می‌شود.
    """
...
class HoldingOut(BaseModel):
    """یک دارایی به‌همراه ارزش امروزش.

    price/value عمداً «ذخیره‌شده» نیستند؛ هر بار از قیمت روز حساب می‌شوند.
    priced=false یعنی نماد این ردیف در دادهٔ امروز بازار پیدا نشد (حذف/تغییر
    نام در بالادست یا خطای موقت). در آن حالت value صفر است ولی این «صفر» با
    «ارزش صفر» فرق دارد — کلاینت باید به‌جای عدد، هشدار نشان دهد. برای همین
    پرچم صریح داریم و صفر خاموش برنمی‌گردانیم.
    """
    ...
    quantity: Decimal
    created_at: datetime
    price: float          # قیمت واحد در آخرین اسنپ‌شات
    value: float          # quantity × price
    priced: bool          # false = قیمت امروز پیدا نشد
    ...
    def of(
        ...
        """unit از قیمت زنده می‌آید و فقط اگر نبود به عکس ذخیره‌شده برمی‌گردیم.

        اگر همیشه واحد ذخیره‌شده را نشان می‌دادیم، تغییر واحد در بالادست
        (مثلاً تومان → ریال) عدد تازه را کنار برچسب کهنه می‌گذاشت — یعنی
...
class HoldingListOut(BaseModel):
    ...
    بتواند بگوید «۱ مورد قیمت ندارد» به‌جای نمایش جمع گمراه‌کننده.
    """

    items: list[HoldingOut]
    total: float
    unpriced: int
    stale: bool           # true = قیمت‌ها از کش قدیمی آمده‌اند
