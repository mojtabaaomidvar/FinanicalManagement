"""روت دارایی‌های بازاری — مقدار طلا/ارز/رمزارز/سهم کاربر.

همهٔ روت‌ها get_tenant_member می‌گیرند تا زمینهٔ RLS خانواده ست شود (fail-closed).
خروجی فهرست شامل قیمت و ارزش امروز است؛ هیچ مبلغی در دیتابیس نیست.
"""
...
def add_holding(
    ...
    # پاسخ ساخت عمداً بی‌قیمت است (price=0, priced=false): کلاینت بلافاصله
    # فهرست را دوباره می‌گیرد و قیمت‌ها یک‌جا می‌آیند. اگر این‌جا هم قیمت
    # می‌گرفتیم، هر «افزودن» یک بار اضافهٔ ارزش‌گذاری داشت بی‌آنکه لازم باشد.
...
def update_holding(
    ...
    # مثل افزودن، بی‌قیمت برمی‌گردد؛ فهرست بعدی قیمت‌ها را یک‌جا می‌آورد.
