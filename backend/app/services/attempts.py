"""گیت مشترک ضد brute-force روی جدول auth_attempts.

این کمک‌تابع‌ها هستهٔ مشترک «قفل موقت» هستند که هم مسیر رمز عبور و هم مسیر OTP
از آن‌ها استفاده می‌کنند (همان پنجره و همان شمارنده — دقیقاً مثل schema.sql).

نکتهٔ حیاتی تراکنش (بهبود نسبت به نسخهٔ Postgres): تلاش ناموفق باید «commit» شود
تا با raise خطای بعدی به عقب برنگردد. در نسخهٔ RPC قدیمی، auth_login پس از درج
...
def recent_fail_count(db: Session, phone: str) -> int:
    """شمار تلاش‌های ناموفق این شماره در پنجرهٔ ضد brute-force."""
...
def record_attempt(db: Session, phone: str, ok: bool) -> None:
    ...
    - موفق: همهٔ تلاش‌های ناموفق این شماره پاک می‌شوند (کاربری که چند بار اشتباه
