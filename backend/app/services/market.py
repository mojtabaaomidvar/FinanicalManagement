"""سرویس بازار — پروکسی کش‌شدهٔ BrsApi (قیمت طلا/ارز/رمزارز + شاخص بورس + جست‌وجوی سهم).

چرا از بک‌اند و نه مستقیم از کلاینت: کلید API نباید در باندل PWA لو برود و
محدودیت CORS هم پیش نمی‌آید. کش سرور-سمتی با TTL (پیش‌فرض ۵ دقیقه) سقف
پلن رایگان BrsApi (۱۵۰۰ درخواست/روز برای طلا/ارز و ۱۰۰۰ برای شاخص) را با
خیال راحت نگه می‌دارد؛ بدترین حالت ~۲۸۸ درخواست/روز است.

سه دستهٔ قیمت از یک فراخوانی Gold_Currency می‌آیند و همان‌جا تفکیک می‌شوند:
طلا/سکه، ارز، و رمزارز. رمزارز تا ۲۰۲۶-۰۹-۱۲ دور ریخته می‌شد؛ حالا دستهٔ
سوم خروجی است (هزینهٔ اضافه ندارد، چون داده در همان پاسخ بود).

خطای بالادست: اگر کش منقضی‌شده‌ای باشد، همان با stale=true برگردانده می‌شود
(کاربر قیمت کمی قدیمی می‌بیند، نه خطا). فقط وقتی هیچ داده‌ای در کار نیست
AppError پرتاب می‌شود.

دقت داده: خروجی مستندات‌شدهٔ BrsApi اعداد را گاه رشته می‌فرستد و منفی را
پسوند می‌کند («33000.22-»)؛ _parse_num همهٔ این حالت‌ها را تحمل می‌کند.

جست‌وجوی تک‌سهم: فهرست کامل نمادهای بورس/فرابورس یک‌جا از AllSymbols.php
گرفته و روی سرور کش می‌شود، بعد همان‌جا فیلتر می‌شود. پس جست‌وجوی کاربر
هیچ درخواست تازه‌ای به بالادست نمی‌زند (سهمیه‌ی رایگان با تایپ کاربر
...
# مهلت هر فراخوانی بالادست. عمداً ۶ ثانیه و نه ۱۰: اسنپ‌شات در بدترین حالت
# دو فراخوانی پشت‌سرهم دارد و کلاینت (restClient.ts) در ۱۵ ثانیه می‌بُرد؛
# ۲×۶ + سرباری < ۱۵ می‌ماند، پس کاربر «خطای شبکه» نمی‌بیند.
_TIMEOUT = httpx.Timeout(6.0)

# عامل کاربر مرورگرمانند — الزامی، نه احتیاط. مستند خود BrsApi می‌گوید
# یوزرایجنت کتابخانه‌های پایتون را «فایروال ۶جی» مسدود می‌کند، پس بدون این
# هدر همهٔ فراخوان‌ها 403 Forbidden می‌شوند حتی با کلید کاملاً معتبر.
# رشتهٔ زیر و Accept، عیناً همان نمونهٔ رسمی مستند BrsApi است.
_USER_AGENT = (
...
# نام‌های رمزارز از فهرست رسمی BrsApi. تا پیش از این این‌ها دور ریخته
# می‌شدند؛ حالا دستهٔ سوم خروجی‌اند (خواستهٔ محصول، ۲۰۲۶-۰۹-۱۲).
_CRYPTO_NAMES = {
...
# تور ایمنی رمزارز. فهرست بالا دستی است و BrsApi هر وقت سکهٔ تازه‌ای اضافه
# کند در آن نیست؛ تا دیروز بی‌اهمیت بود (همه‌چیز ناشناخته در «ارز» می‌افتاد و
# دیده نمی‌شد) ولی حالا که رمزارز بخش جداست، یک سکهٔ جدید اشتباهاً کنار دلار
# می‌نشیند. پس نماد لاتین را هم می‌سنجیم: نمادهای رمزارز در خروجی BrsApi
# لاتین‌اند (BTC, ETH…) در حالی که ارز/طلا نماد فارسی یا کد ارزی دارند.
_CRYPTO_SYMBOLS = {
...
# یکسان‌سازی حرف‌های هم‌شکل عربی/فارسی + حذف اعراب — فقط برای جست‌وجو.
...
def _norm_search(s: str) -> str:
    """نرمال‌سازی جست‌وجو: حرف‌های هم‌شکل عربی/فارسی، ارقام، فاصله و نیم‌فاصله.
...
def _is_crypto(name: str, symbol: str = "") -> bool:
    """رمزارز است؟ اول نام فارسی، بعد نماد لاتین (تور ایمنی سکه‌های تازه)."""
    n = _norm(name)
...
def _is_gold(name: str) -> bool:
    """فلز گران‌بها (طلا/سکه/نقره/پلاتین/انس) — وگرنه در فهرست «ارز» می‌افتد."""
    n = _norm(name)
    return any(k in n for k in ("طلا", "سکه", "نقره", "پلاتین", "پالادیوم", "انس"))


def _parse_num(v: object) -> float:
    """عدد تحمل‌پذیر: int/float/«859,000»/«33000.22-»/«۱٬۲۳۴» → float."""
    ...
    if not s:
        return 0.0
    neg = s.endswith("-")  # فرمت BrsApi: منفی گاهی پسوند است
    if neg:
        s = s[:-1]
    s = s.lstrip("+")
    try:
...
def _unwrap(payload: dict, *keys: str) -> object:
    """اگر پاسخ در پوشش {successful, data, …} باشد، هستهٔ داده را بیرون می‌کشد."""
    for k in keys:
        v = payload.get(k)
        if v is not None:
            return v
...
def _upstream_payload(payload: object) -> object:
    """پاسخ خطای BrsApi ({successful:false, message_error}) → AppError؛ وگرنه هسته."""
...
# کلیدهای بخش‌بندی خروجی Gold_Currency. BrsApi گاهی فهرست تخت می‌دهد و گاهی
# شیء بخش‌بندی‌شده؛ مستند خودشان هم پارامتر `section=gold,currency` دارد.
...
def _split_prices(
    ...
    """خروجی Gold_Currency → (طلا، ارز، رمزارز). هر دو شکل ممکن را می‌فهمد.

    چرا این تابع لازم شد: پیش‌تر کد فقط `isinstance(payload, list)` را می‌پذیرفت و
    اگر پاسخ شیء بخش‌بندی‌شده بود آن را **بی‌صدا** به [] تبدیل می‌کرد — نه خطا،
    نه پرچم stale. نتیجه‌اش صفحه‌ای بود که بدون هیچ توضیحی طلا و ارز را نشان
    نمی‌داد، در حالی که بورس (از اندپوینت دیگری) سالم می‌آمد.

    وقتی پاسخ بخش‌بندی‌شده باشد، خود کلید بخش معتبرترین منبع دسته‌بندی است و
    به حدس نام/نماد ترجیح داده می‌شود. `_is_crypto` فقط داخل بخش «ارز» اجرا
    می‌شود تا اگر بالادست رمزارز را همان‌جا ریخته بود بیرون کشیده شود.
    """
    # شکل ۱ — فهرست تخت: دسته‌بندی از روی نام/نماد
    ...
    # شکل ۲ — شیء بخش‌بندی‌شده: کلید بخش حرف آخر را می‌زند
    if isinstance(payload, dict):
        ...
        for bucket, keys in _SECTION_KEYS.items():
            for k in keys:
                if k in payload:
                    ...
                    break
        if hit:
            # رمزارزی که بالادست داخل «ارز» گذاشته باشد را جدا می‌کنیم
    ...
    # شکل ناشناخته — عمداً بلند خطا می‌دهیم. سکوت همان اشکالی بود که ساعت‌ها
    ...
    raise AppError("SERVER", f"شکل پاسخ طلا/ارز BrsApi ناشناخته است — {shape}", 502)
...
def _map_bourse(payload: object) -> BourseOut | None:
    """خروجی Index.php?type=1 → BourseOut؛ ساختار خام یا پوشش‌شده را تحمل می‌کند."""
    core = payload
...
def _map_stock(row: dict) -> StockOut:
    """یک ردیف AllSymbols.php → StockOut.

    نام پارامترها همان متغیرهای فیلتر tsetmc است (l18 نماد، l30 نام کامل،
    pl آخرین معامله، pc قیمت پایانی، tno/tvol/tval آمار معاملات). چون
...
def _fetch(url: str, key: str, extra: dict | None = None) -> object:
    ...
    if extra:
        params.update(extra)
    try:
        # بدون این دو هدر، BrsApi با 403 پاسخ می‌دهد (فایروال ۶جی، UA پایتون).
        ...
        # بدنهٔ پاسخ را نگه می‌داریم: BrsApi دلیل رد را همان‌جا می‌نویسد
        # (کلید باطل، اتمام سهمیه، پلن بدون دسترسی…). بدون این، خطا
        # قابل عیب‌یابی نیست. کلید از متن پاک می‌شود.
        body = _scrub((e.response.text or "").strip()[:200], key)
        status = e.response.status_code
        if status in (401, 403):
            raise AppError(
                "SERVER",
                f"BrsApi دسترسی را رد کرد ({status}) — {body or 'بدون توضیح'}",
                502,
            ) from e
...
_symbols_cache: _Entry | None = None  # list[StockOut] — فهرست کامل نمادها


def _unpack_prices(data: object) -> tuple[list[MarketItemOut], list[MarketItemOut], list[MarketItemOut]]:
    """کش قیمت‌ها را باز می‌کند و شکل دوتایی قدیمی را هم تحمل می‌کند.

    چرا: پیش از افزودن رمزارز، کش تاپل (gold, currency) بود. اگر پروسه‌ای
    با کش گرم دوتایی این کد را اجرا کند، unpack سه‌تایی ValueError می‌دهد و
...
# قفل نوشتن کش. عمداً قفل جدا برای هر بخش نداریم: نوشتن‌ها کوتاه‌اند.
_lock = threading.Lock()

# قفل «یک نفر برود بیاورد» — جلوی هجوم هم‌زمان به بالادست را می‌گیرد.
# بدون این، چند درخواست هم‌زمان روی کش سرد هرکدام یک فراخوانی BrsApi
...
def get_market_snapshot(settings: Settings | None = None) -> MarketSnapshotOut:
    """اسنپ‌شات بازار — از کش تازه، وگرنه فراخوانی بالادست.

    بخش‌ها مستقل کش می‌شوند: اگر یکی از دو اندپوینت BrsApi خطا بدهد، بخش دیگر
    تازه می‌ماند و نسخهٔ کش قدیمی همان بخش با stale=true برگردانده می‌شود.
    """
    ...
    if _fresh(_prices_cache, ttl):
        ...
        # فقط یک thread می‌رود بالادست؛ بقیه پشت قفل می‌مانند و بعد از آزاد شدن
        # کش تازه‌ای که همان نفر نوشته را برمی‌دارند (دوباره _fresh را می‌سنجیم).
            ...
            if _fresh(_prices_cache, ttl):
                ...
                try:
                    ...
                    if not gold and not currency and not crypto:
                        raise AppError("SERVER", "طلا/ارز BrsApi خالی برگشت.", 502)
                    with _lock:
                        _prices_cache = _Entry((gold, currency, crypto))
                except AppError:
                    # کش قبلی بهتر از هیچ است، ولی باید «قدیمی» علامت بخورد.
                    # اگر کشی هم نباشد سه فهرست خالی می‌مانند و گارد پایین‌تر
    ...
    # زمان نمایشی = *قدیمی‌ترین* بخش موجود، نه تازه‌ترین.
    # با max() اگر قیمت‌ها تازه ولی شاخص کهنه بود، کاربر «الان» می‌دید و
    # کهنگی شاخص پنهان می‌ماند؛ min() صادق‌تر است.
    with _lock:
...
# ── جست‌وجوی تک‌سهم ──────────────────────────────────────────
# فهرست کامل نمادها یک‌جا گرفته و کش می‌شود، بعد روی سرور فیلتر می‌شود.
# TTL کوتاه‌تر از اسنپ‌شات است چون قیمت سهم در ساعت بازار تندتر عوض می‌شود،
# ولی همچنان کش است تا تایپ‌کردن کاربر سهمیهٔ رایگان را نسوزاند.
...
def _load_symbols(cfg: Settings) -> tuple[list[StockOut], float, bool]:
    ...
    with _fetch_locks["symbols"]:
        ...
        try:
            ...
            if not stocks:
                raise AppError("SERVER", "فهرست نمادها خالی برگشت.", 502)
            entry = _Entry(stocks)
            with _lock:
                _symbols_cache = entry
            return stocks, entry.at, False
        except AppError:
            # کش منقضی بهتر از خطا است — قیمت کمی قدیمی، ولی قابل استفاده.
...
def _score(stock: StockOut, q: str) -> int | None:
    ...
    ترتیب عمدی است: نماد دقیق → نمادی که با q شروع می‌شود → نماد شامل →
    نام شرکت. وگرنه جست‌وجوی «فولاد» اول شرکت‌هایی را می‌آورد که «فولاد»
    جایی در وسط نامشان است و خود نماد «فولاد» گم می‌شود.
    """
...
def search_stocks(
    ...
    """جست‌وجوی نماد بورس/فرابورس روی فهرست کش‌شدهٔ سرور.

    فیلتر سمت سرور است: کلاینت فقط q را می‌فرستد و چند ردیف مرتبط می‌گیرد،
    نه چند هزار نماد. قیمت برگشتی همان لحظه‌ای است که سرور فهرست را گرفته و
    ...
    # حرفی‌بودن لحظه‌ای «خطا» نیست. ضمناً این‌جا اصلاً سراغ بالادست نمی‌رویم.
    if len(q) < 2:
        with _lock:
    ...
    # رتبه اول، بعد حجم معاملات نزولی: بین هم‌رتبه‌ها سهم پرمعامله‌تر
...
# ── قیمت‌گذاری دارایی‌های کاربر ─────────────────────────────
# سرویس holdings مقدار کاربر را دارد و قیمت امروز را از این‌جا می‌گیرد.
# عمداً هیچ درخواست تازه‌ای به بالادست اضافه نمی‌کند: همان دو کشی که
# اسنپ‌شات و جست‌وجو از آن می‌خوانند این‌جا هم استفاده می‌شوند.


def price_key(kind: str, symbol: str, name: str = "") -> str:
    """کلید یکتا برای نگاشت قیمت.

    عمومی است چون سرویس holdings هم برای جست‌وجو در نگاشت به آن نیاز دارد؛
    اگر خصوصی می‌ماند، دو تعریف موازی از «کلید» می‌داشتیم که بی‌صدا از هم
    جدا می‌افتادند.

    kind در کلید هست چون نام‌ها بین دسته‌ها تکراری‌اند («طلا» هم در فلزات هست
    هم می‌تواند نام نمادی در بورس باشد)؛ بدون kind یک هم‌نامی اتفاقی قیمت
    دستهٔ دیگر را برمی‌گرداند و کاربر عدد بی‌ربط می‌بیند.
    """
...
def _is_tether_row(name: str) -> bool:
    """ردیف «دلار تتر» در سبد ارز است؟

    بالادست تتر را دو جا می‌گذارد: در سبد رمزارز با قیمت جهانی (~۱ دلار) و
    در سبد ارز با نرخ تومانی بازار ایران. اسم دومی «دلار تتر» است و در
    `_usd_toman` به‌جای نرخ دلار برداشته می‌شد.
    """
    return "تتر" in _norm_search(name)


def _usd_toman(items: list[MarketItemOut]) -> float:
    """نرخ دلار به تومان از همان اسنپ‌شات ارز — ۰ یعنی پیدا نشد.

    بعضی ردیف‌های بالادست (به‌ویژه رمزارز) قیمت دلاری می‌دهند و بدون این نرخ
    نمی‌شود آن‌ها را با بقیه جمع زد. هیچ درخواست تازه‌ای اضافه نمی‌کند چون
    اسنپ‌شات هر سه سبد را با هم می‌آورد.

    ردیف تتر عمداً کنار گذاشته می‌شود: نامش «دلار» دارد و شرط دوم این تابع
    آن را برمی‌داشت. الان مسکوت است چون شرط اول («دلار آمریکا») جلوتر
    می‌گیرد، ولی روزی که بالادست آن نام را عوض کند، نرخ تتر بازار ایران
    به‌جای دلار در ارزش‌گذاری *همهٔ* دارایی‌ها می‌نشست — خطایی که کاربر
    می‌بیند و علتش را در این فایل نمی‌جوید.
    """
    real = [i for i in items if not _is_tether_row(i.name)]
    row = next((i for i in real if _norm_search(i.name) == _norm_search("دلار آمریکا")), None)
    if row is None:
        row = next((i for i in real if "دلار" in _norm_search(i.name)), None)
    if row is None or row.price <= 0:
        return 0.0
    # واحد خود دلار فقط ریال/تومان است؛ عمداً بازگشتی حل نمی‌شود.
...
def _to_toman(price: float, unit: str, usd_rate: float) -> float:
    """قیمت یک قلم را به تومان برمی‌گرداند؛ خروجی ۰ یعنی «واحد را نمی‌شناسیم».

    صفر برگشتی عمدی است: سرویس holdings همین حالا هم قیمت ≤۰ را
    priced=false می‌گیرد، پس ردیف با واحد ناشناخته به‌جای آلوده‌کردن جمع،
    «قیمت ندارد» نشان داده می‌شود — همان الگوی fail-closed بقیهٔ این فایل.
    """
    if price <= 0:
        return 0.0
    n = _norm_search(unit)
    if not n:
        # نبود واحد را تومان می‌گیریم: هم واحد پایهٔ خود اپ است هم پیش‌فرض
...
def price_lookup(
    ...
    """نگاشت «کلید → (قیمت تومانی، واحد)» + پرچم کهنگی.

    قیمت همیشه به **تومان** یکسان‌سازی می‌شود و واحد برگشتی هم همیشه «تومان»
    است. این عمدی است: بالادست سهم را به ریال، طلا/ارز را به تومان و گاهی
    رمزارز را به دلار می‌دهد، و سرویس holdings این عددها را در یک `total`
    جمع می‌زند. بدون یکسان‌سازی، سبدی که هم سهم داشت هم سکه، جمع بی‌معنایی
    می‌ساخت که برای بخش سهمش ۱۰ برابر بود.

    فقط دسته‌هایی که کاربر واقعاً دارایی‌شان را دارد بار می‌شوند: اگر کسی هیچ
    سهمی ندارد، فهرست چندهزارتایی نمادها اصلاً گرفته نمی‌شود.

    هر قلم با دو کلید ثبت می‌شود (نماد و نام) چون بالادست برای بعضی ردیف‌های
    طلا/سکه نماد خالی می‌فرستد و تنها چیز پایدار نام است؛ کلاینت هم ممکن است
    هرکدام را ذخیره کرده باشد.

    خطای بالادست این‌جا بلعیده نمی‌شود اما کشنده هم نیست: اگر داده‌ای نبود
    نگاشت خالی برمی‌گردد و سرویس holdings ردیف‌ها را priced=false علامت
    می‌زند — یعنی کاربر فهرست دارایی‌هایش را می‌بیند، فقط بدون عدد ارزش.
    """
    cfg = settings or get_settings()
    ...
    if spot:
        try:
            ...
            # از سبد ارز خوانده می‌شود حتی وقتی کاربر ارزی ندارد: اسنپ‌شات
            ...
            for kind in spot:
                for it in buckets[kind]:
                    ...
                    for key in (
                        ...
                        # نماد خالی کلید «kind:» می‌سازد که به هر ردیف بی‌نام
    ...
    if "stock" in kinds:
        try:
            ...
            for s in stocks:
                # قیمت سهم در بالادست ریال است (هم‌سان با UI جست‌وجو) و
