#!/usr/bin/env bash
# ── بازگردانی دیتابیس خانه‌یار از یک فایل بکاپ ──
#
# استفاده:
#     ./restore.sh /var/backups/khaneyar/db-YYYYMMDD-HHMMSS.sql.gz
#
# هشدار: این روی دیتابیس فعلی می‌نویسد. برای بازگردانی تمیز (فاجعه/سرور نو)،
# بهتر است دیتابیس خالی باشد. تست دوره‌ای این اسکریپت = تنها راه اطمینان از بکاپ.
...
echo "restoring '$f' → دیتابیس '$POSTGRES_DB' (دادهٔ فعلی بازنویسی می‌شود)"
