#!/usr/bin/env bash
# تشخیص ۴۰۳ BrsApi — کلید را از deploy/.env می‌خواند و هرگز چاپ نمی‌کند.
#
# چرا لازم شد: لاگ سرور نشان داد هر دو فراخوانی بالادست «403 Forbidden»
# می‌گیرند. علت (تأییدشده از مستند خود BrsApi): یوزرایجنت کتابخانه‌های
# پایتون توسط «فایروال ۶جی» مسدود می‌شود. این اسکریپت همان را اثبات می‌کند:
# یک فراخوان با UA پایتون، یکی با UA مرورگر، و یکی با کلید عمداً غلط تا
# پاسخ «کلید بد» از پاسخ «UA بد» قابل تفکیک باشد.
# خروجی فقط کد وضعیت + ۲۰۰ کاراکتر اول بدنه است.
#
...
probe() {
  local label="$1"; shift
  # -s بی‌صدا، بدنه در فایل موقت، کد وضعیت جدا. کلید در خروجی نمی‌آید.
...
echo "=== آزمون ۱: بدون هدر خاص (مثل httpx پیش‌فرض) ==="
probe "بدون User-Agent" -H 'User-Agent: python-httpx/0.27'

echo "=== آزمون ۲: با User-Agent مرورگر ==="
probe "با UA مرورگر" -H "User-Agent: ${UA_BROWSER}" -H 'Accept: application/json'

echo "=== آزمون ۳: کلید عمداً غلط (برای مقایسه) ==="
bad_body="$(mktemp)"
: > "$bad_body"   # تضمین وجود فایل حتی اگر curl شکست بخورد
...
echo "=== تفسیر ==="
echo "• انتظار اصلی: ۱ = 403 و ۲ = 200 ⇒ علت فقط User-Agent است."
echo "  مستند خود BrsApi می‌گوید UA پایتون را «فایروال ۶جی» مسدود می‌کند."
echo "• اگر ۲ هم 403 داد ⇒ این بار پای کلید/پلن است، نه هدر؛ بدنه را بخوان."
echo "• اگر بدنه از «پلن» یا «اشتراک» حرف زد ⇒ اندپوینت در پلن فعلی نیست."
