#!/usr/bin/env python3
"""خودآزمون بدون‌دیتابیس پارسر پیامک (فاز ۸، کار #116).

نمونه‌های واقعی پیامک بانکی ایرانی را پارس می‌کند و خروجی را با انتظار می‌سنجد؛
به‌علاوه چند تبدیل جلالی↔میلادی را مستقیم بررسی می‌کند. بدون دیتابیس/شبکه.
...
# ── ۱) تبدیل تقویم (باید با jalali.ts یکی باشد) ─────────────────
...
# کارت‌به‌کارت سامان (هزینه) با «مانده»
check(
    "کارت‌به‌کارت سامان",
    "بانک سامان\nکارت به کارت\nمبلغ 800,000\nمانده 3,200,000\n1404/06/20",
    type="expense",
...
# حقوق صادرات (درآمد) — دستهٔ salary
check(
    "حقوق صادرات",
    "بانک صادرات\nحقوق واریز شد\nمبلغ 45,000,000\nموجودی 46,200,000\n1404/07/01",
    type="income",
...
# خرید پارسیان با ارقام فارسی (آزمون to_en) — دستهٔ shopping
check(
...
# پیامک اطلاعی صرف موجودی — نوع نامشخص، مبلغ=موجودی (محدودیت شناخته‌شده)
check(
...
# مرز خالی‌بودن (هم‌سان با sms-parser.ts):
#   - رشتهٔ کاملاً خالی → None
#   - رشتهٔ فقط‌فاصله → شیء با همهٔ فیلدها None (TS هم trim نمی‌کند؛ خالی‌بودن
