#!/usr/bin/env python3
"""راستی‌آزمایی ایستای ابزار مهاجرت فاز ۱۰ (کار #94) — بدون دیتابیس/شبکه.

ثابت‌ها و توابع خالص backend/migration/migrate_essentials.py را می‌سنجد:
  • national_id همیشه کنار می‌رود.
  • members.avatar_url اجباراً NULL می‌شود.
  • password_hash و phone در سورس اجباری‌اند.
  • ترتیب جدول‌ها امن کلید خارجی است.
  • جدول‌های RLS-محافظت‌شده شامل families/members نیستند و همه در مجموعهٔ ضروری‌اند.
  • FRESH_TABLES با جدول‌های ضروری هم‌پوشانی ندارد.
  • normalize_url و carry_columns درست کار می‌کنند.

psycopg در این محیط لازم نیست؛ با یک ماژول ساختگی جایگزین می‌شود تا import ماژول کار کند.
...
# ── جایگزینی ساختگی psycopg تا import ماژول مهاجرت بدون نصب درایور کار کند ──
...
check("password_hash در سورس members اجباری است",
      "password_hash" in m.REQUIRED_SOURCE_COLUMNS.get("members", set()))
check("phone در سورس members اجباری است",
...
check("۶ جدول ضروری تعریف شده", len(et) == 6 and len(set(et)) == 6)

check("RLS_GUARDED زیرمجموعهٔ جدول‌های ضروری است",
      m.RLS_GUARDED_ESSENTIALS <= set(et))
check("families/members در RLS_GUARDED نیستند (بوت‌استرپ هویت)",
...
check("card_bins جزو جدول‌های ضروری نیست", "card_bins" not in et)
check("card_bins جزو FRESH نیست (مرجع است، نه دادهٔ کاربر)",
      "card_bins" not in m.FRESH_TABLES)
check("کلید تکرار card_bins برابر bin است (نه id)",
      m.CONFLICT_KEY.get("card_bins") == "bin")
check("کلید پیش‌فرض تکرار برای بقیه id است",
...
check("postgresql:// بدون تغییر می‌ماند",
      m.normalize_url("postgresql://u:p@h/db") == "postgresql://u:p@h/db")

print("\n── carry_columns ──────────────────────────")
# تارگت: id, family_id, national_id, avatar_url, kind(تازه)   سورس: بدون kind، همراه legacy_col
...
check("ستون تازهٔ تارگت (kind، نبود در سورس) کنار می‌رود", "kind" not in carry)
check("ستون فقط-سورس (legacy_col) وارد نمی‌شود", "legacy_col" not in carry)
check("ترتیب carry از تارگت حفظ می‌شود", carry == ["id", "family_id", "avatar_url"])
