/* رله محلی Maven v4 — استریم مستقیم curl → Gradle
   ...
   v4: stdout کرل مستقیم به سوکت Gradle استریم می‌شود — داده دائماً
   جریان دارد (سوکت‌تایم‌اوت گرتل نمی‌سوزد) و اگر کرل وسط راه بمیرد،
...
const server = http.createServer((req, res) => {
  ...
  const tryStream = (urlIdx) => {
    ...
    curl.on("close", (code) => {
      ...
      /* استریم کرده‌ایم — اگر کرل با کد ناصفر مرده و چیزی نفرستاده → میرور بعدی */
      if (code !== 0 && bytes === 0) {
