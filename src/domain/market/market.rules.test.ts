...
describe("findDollar", () => {
  ...
  it("بدون «دلار آمریکا»، اولین نام شامل «دلار»", () => {
...
describe("usdToman", () => {
  ...
  it("واحد خالی = تومان", () => {
    expect(usdToman([item("دلار آمریکا", { price: 90000, unit: "" })])).toBe(
      90000,
    );
  });

  it("نبود دلار یا قیمت صفر → ۰", () => {
...
describe("toToman", () => {
  ...
  it("یای عربی در «ريال» هم شناخته می‌شود", () => {
    // بالادست هر دو املا را فرستاده؛ تطبیق خام این ردیف را بی‌قیمت می‌کرد
    expect(toToman(5000, "ريال", 0)).toBe(500);
    expect(toToman(5000, " ریال ", 0)).toBe(500);
  });

  it("دلار با نرخ روز ضرب می‌شود", () => {
    expect(toToman(2, "دلار", 90000)).toBe(180000);
    expect(toToman(2, "USD", 90000)).toBe(180000);
    expect(toToman(2, "$", 90000)).toBe(180000);
  });

  it("دلار بدون نرخ → ۰ (نه عدد غلط)", () => {
    expect(toToman(2, "دلار", 0)).toBe(0);
  });

  it("واحد خالی تومان فرض می‌شود", () => {
