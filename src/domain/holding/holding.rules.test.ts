...
describe("parseQuantityInput", () => {
  ...
  it("ورودی خالی/نامعتبر صفر می‌شود، نه NaN", () => {
...
describe("liveFormatQuantity", () => {
  it("نقطهٔ در حال تایپ حفظ می‌شود", () => {
...
describe("validateQuantity", () => {
  ...
  it("مقداری که به صفر گرد می‌شود، پیش سرور رد می‌شود", () => {
    // وگرنه Postgres گردش می‌کند و CHECK(quantity > 0) با خطای مبهم می‌ترکد
    expect(validateQuantity(0.0000001).error).toBe("ZERO_QUANTITY");
  });

  it("بزرگ‌تر از ظرفیت ستون رد می‌شود", () => {
    expect(validateQuantity(1e13).error).toBe("HUGE_QUANTITY");
  });
});

describe("roundQuantity", () => {
  it("به شش رقم اعشار گرد می‌کند", () => {
...
describe("validateHoldingInput", () => {
  ...
  it("نوع ناشناخته", () => {
    expect(
      validateHoldingInput(input({ kind: "bogus" as HoldingInput["kind"] }))
        .error,
    ).toBe("INVALID_KIND");
  });

  it("نام خالی یا خیلی بلند", () => {
    expect(validateHoldingInput(input({ name: "  " })).error).toBe(
      "INVALID_NAME",
    );
  ...
  it("مقدار نامعتبر از همان مسیر validateQuantity رد می‌شود", () => {
    expect(validateHoldingInput(input({ quantity: 0 })).error).toBe(
      "ZERO_QUANTITY",
    );
  });
});
...
/* این دو تابع آینهٔ قاعدهٔ سرورند:
   ...
   ویرایش، ردیف تکراری ساخته می‌شود. */
describe("effectiveSymbol", () => {
  it("نماد موجود، فقط trim می‌شود", () => {
    expect(effectiveSymbol("  IRO1FOLD0001 ", "فولاد مبارکه")).toBe(
      "IRO1FOLD0001",
    );
  });

  it("نماد خالی → نام جایش را می‌گیرد", () => {
    // ردیف‌های طلا/سکه در بالادست اغلب نماد ندارند
    expect(effectiveSymbol("", "سکه امامی")).toBe("سکه امامی");
    expect(effectiveSymbol("   ", "سکه امامی")).toBe("سکه امامی");
  });

  it("نماد بلندتر از ۴۰ بریده می‌شود — دقیقاً مثل سرور", () => {
    expect(effectiveSymbol("x".repeat(50), "نام")).toBe("x".repeat(40));
  });

  it("نام بلند در حالت جایگزینی بریده نمی‌شود", () => {
...
describe("findHolding", () => {
  ...
  it("ردیف موجود را با نماد خام می‌یابد", () => {
    expect(findHolding(items, "stock", "فولاد", "فولاد مبارکه")?.symbol).toBe(
      "فولاد",
    );
  });

  it("نماد خالی با نام تطبیق داده می‌شود", () => {
    expect(findHolding(items, "gold", "", "سکه امامی")?.symbol).toBe(
      "سکه امامی",
    );
  });

  it("نوع متفاوت تطبیق نمی‌دهد", () => {
    // «فولاد» بورس نباید با دارایی‌ای از نوع طلا یکی شمرده شود
    expect(findHolding(items, "gold", "فولاد", "فولاد")).toBeUndefined();
  });

  it("نماد ناموجود → undefined (یعنی «تازه است»)", () => {
    expect(findHolding(items, "crypto", "ETH", "اتریوم")).toBeUndefined();
  });

  it("نماد و نام خالی → undefined، نه تطبیق تصادفی", () => {
