/* قواعد دارایی — اعتبارسنجی و قالب‌بندی خالص (بدون وابستگی به React/IO).

   چرا پارسر جدا از parseAmountInput؟ آن تابع عمداً هر چیز غیر رقم را حذف
   می‌کند (`[^\d]`)، پس «۲٫۵ گرم» را ۲۵ می‌خواند. مقدار دارایی ذاتاً اعشاری
   است (۲٫۵ گرم طلا، ۰٫۰۳ بیت‌کوین)، پس این‌جا جداکنندهٔ اعشار را نگه
   می‌داریم. ستون دیتابیس Numeric(18,6) است و این قواعد دقیقاً با همان
...
/** واحد شمارش هر دسته — کنار عدد مقدار می‌نشیند («۲ عدد»، «۳۰۰ واحد») */
...
/* رشتهٔ ورودی → شکل استاندارد انگلیسی با نقطهٔ اعشار.
   ٫ (U+066B) جداکنندهٔ اعشار فارسی است و به نقطه تبدیل می‌شود؛ ولی
   ٬ (U+066C) و ، (U+060C) جداکنندهٔ هزارند و باید حذف شوند — اگر این دو
   را هم اعشار می‌گرفتیم، «۱٬۲۰۰» به‌جای ۱۲۰۰ عدد ۱٫۲ خوانده می‌شد. */
function normalize(raw: string): string {
  return toEn(String(raw))
    .replace(/٫/g, ".") // ٫ اعشار فارسی → نقطه
    .replace(/[٬،,\s_]/g, "") // ٬ ، و کاما/فاصله: جداکنندهٔ هزار
    .replace(/[^\d.]/g, ""); // هر چیز دیگر
}
...
/** قالب‌بندی زندهٔ ورودی: ارقام فارسی + نگه‌داشتن جداکنندهٔ اعشار در حال تایپ.
    «۲.» نباید وسط تایپ حذف شود وگرنه کاربر نمی‌تواند اعشار وارد کند. */
...
/** عدد → نمایش فارسی، بدون صفر انتهایی: 2 → «۲» ، 2.5 → «۲٫۵» */
...
/** مقدار را به دقت ستون گرد می‌کند.
    پیش از ارسال لازم است: اگر عدد درازتری برود، Postgres خودش گرد می‌کند
    و «۰٫۰۰۰۰۰۰۱» به صفر تبدیل می‌شود و بعد CHECK quantity > 0 با خطای
    مبهم دیتابیس می‌ترکد — بهتر است همین‌جا صریح رد شود. */
...
/** اعتبارسنجی مقدار — کدهای خطا با پیام‌های یوزکیس نگاشت می‌شوند */
...
/** نمادی که واقعاً ذخیره می‌شود.
    ...
    سرور نماد خالی را با نام پر می‌کند (بعضی ردیف‌های طلا/سکه در بالادست
    نماد ندارند و نام تنها چیز پایدارشان است). همان قاعده این‌جا تکرار
    می‌شود چون کلاینت باید بتواند ردیف موجود را پیدا کند؛ اگر دو طرف
    یکی نبودند، کاربر روی «سکهٔ امامی» می‌زد و به‌جای ویرایش ردیف موجود،
    ردیف تکراری ساخته می‌شد. */
export function effectiveSymbol(symbol: string, name: string): string {
  return (symbol || "").trim().slice(0, 40) || (name || "").trim();
}

/** ردیف موجود همین قلم را پیدا می‌کند — نبودش یعنی «تازه است».
    مقایسه با نماد مؤثر انجام می‌شود، نه نماد خام. */
