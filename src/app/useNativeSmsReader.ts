/* چسب اپ: خواندن نیتیو پیامک روی اندروید.
   ...
   می‌دهد و هر متن را از مسیر ingestRawSms به مخزن می‌فرستد (سرور تصمیم می‌گیرد بانکی
   هست یا نه). روی وب/iOS بی‌صدا رد می‌شود؛ پل فورواردر همچنان فالبک است. */
...
export function useNativeSmsReader(
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        // مجوز: اگر داده نشده، یک‌بار می‌پرسیم؛ رد کاربر = بی‌صدا صرف‌نظر
