/* UI قیف آغازین — پیش از ورود (فارسی، RTL، برند «روشن و آرام»)

   مسیر کاربر تازه: «شروع» → معرفی → چند پرسش سبک → فرم ثبت‌نام
   از‌پیش‌پرشده. کاربر قدیمی روی دکمه‌ی «ورود» می‌زند و مستقیم به فرم
   ورود می‌رود.

   صادق نسبت به قابلیت‌های امروز؛ بدون تیزر ماژول‌های نساخته. */
...
/* ═══ پس‌زمینه‌ی گام خوش‌آمد — آیکون‌های خانه و پول که آرام شنا می‌کنند ═══

   جای آیکون‌ها هر بار تصادفی است و دور مرکز صفحه — یعنی پشت لوگو و
   نوشته‌ها. خوانایی متن با کم‌رنگ بودن آیکون‌ها تأمین می‌شود، نه با
   خالی‌گذاشتن مرکز. */
const BG_ICONS = [
  "i-home",
  "i-wallet",
...
/* چیدمان تصادفی خوشه‌ای دور مرکز.
   مختصات قطبی است نه x/y مستقل: با زاویه‌ی تصادفی و شعاع تصادفی، آیکون‌ها
   دور مرکز حلقه می‌زنند و کش افقی/عمودی نمی‌گیرند. توان ۰٫۷ روی شعاع،
   تراکم را از مرکز به بیرون کمی پخش می‌کند تا وسط توده نشود.
   ضریب ۰٫۹ روی محور y چون صفحه بلندتر از عرض است. */
...
/* چهار قابلیت آماده‌ی امروز — همان چیزی که واقعاً کار می‌کند */
const VALUE_POINTS: { icon: string; title: string; desc: string }[] = [
  {
    icon: "i-users",
    title: "خرج و درآمد همه، یکجا",
    desc: "هر عضو تراکنش‌هایش را ثبت می‌کند و همه در یک نگاه دیده می‌شود.",
  },
  {
    icon: "i-sms",
    title: "ثبت خودکار از پیامک بانک",
    desc: "پیامک تراکنش را می‌خوانَد و خودش هزینه را برایت ثبت می‌کند.",
  },
  {
    icon: "i-piggy",
    title: "بودجه‌ی ماهانه",
    desc: "برای هر دسته سقف بگذار و جلوی خرج اضافه را بگیر.",
  },
  {
    icon: "i-chart",
    title: "گزارش و تقویم شمسی",
    desc: "نمودارها و گزارش ماهانه، بر پایه‌ی تاریخ شمسی.",
  },
];
...
export function OnboardingFeature({
  ...
  /* یک‌بار در هر بار باز شدن قیف قرعه می‌خورد — نه در هر رندر، وگرنه با
     هر بار تایپ‌کردن آیکون‌ها می‌پریدند */
  const bgItems = useMemo(makeBgItems, []);

  const primaryLabel = m.isFirst ? "شروع" : m.isLast ? "ساخت حساب" : "ادامه";

  return (
    <>
      {/* فقط گام خوش‌آمد — گام‌های بعدی فرم دارند و باید آرام بمانند */}
      {m.isFirst ? (
      ...
      <div className="onb-page">
          ...
          {m.step === "welcome" ? (
            <div className="onb-hero">
              ...
              <p className="onb-sub onb-sub-hero">
                مدیریت پول خانواده، با آرامش و با هم؛ همه‌ی خرج‌ها، درآمدها و
                بودجه‌ها یکجا.
              </p>
            </div>
          ) : null}
          ...
          {m.step === "questions" ? (
            <>
              <div className="onb-head">
                ...
                <p className="onb-sub">
                  چند پرسش کوتاه تا اپ را مطابق خانه‌ی شما تنظیم کنیم.
                </p>
              </div>
              ...
              <div className="onb-field">
                <span className="onb-label">واحد پول</span>
        ...
        {/* کاربر قدیمی — جعبه‌ی توخالی، آرام‌تر از دکمه‌ی اصلی */}
