/* اعمال پیش‌نویس قیف آغازین روی حساب تازه‌ساخته‌شده.

   واحد پول پیش از ورود پرسیده می‌شود، ولی تا وقتی عضوی وجود نداشته باشد
   جایی برای نوشتنش نیست. این هوک منتظر می‌ماند تا نشست آماده شود، بعد
   یک‌بار اعمال و پیش‌نویس را پاک می‌کند.

   عمداً «بی‌صدا» است: اگر نوشتن شکست بخورد کاربر را وسط ورود به اپ
   با خطا روبه‌رو نمی‌کنیم — واحد پول همیشه از تنظیمات قابل تغییر است. */
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        /* بی‌صدا — از تنظیمات قابل اصلاح است */
      } finally {
        clearIntroDraft();
      }
    })();
